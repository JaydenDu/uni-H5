...
<head>
    <title>Code coverage report for main/modules/img.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">main/modules</a> img.ts</h1>
        <div class='clearfix'>
...
<span class="cstat-no" title="statement not covered" >    return url</span>
  } else <span class="cstat-no" title="statement not covered" >if (url.startsWith('/')) {</span>
<span class="cstat-no" title="statement not covered" >    return location.origin + url</span>
  } else {
<span class="cstat-no" title="statement not covered" >    return location.origin + location.pathname + url</span>
  }
}
&nbsp;
// https://bugzilla.mozilla.org/show_bug.cgi?id=1607081
function <span class="fstat-no" title="function not covered" >isSVGInFireFox(</span>url: string) {
<span class="cstat-no" title="statement not covered" >  return IS_FIREFOX &amp;&amp; (url.startsWith('data:image/svg+xml') || url.match(/.svg$|/i))</span>
}
&nbsp;
const PLACEHOLDER_SRC = <span class="cstat-no" title="statement not covered" >'https://static.openreplay.com/tracker/placeholder.jpeg'</span>
&nbsp;
...
&nbsp;
  const sendSrcset = <span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >function (i</span>d: number, img: HTMLImageElement): void {</span>
    const { srcset } = <span class="cstat-no" title="statement not covered" >img</span>
<span class="cstat-no" title="statement not covered" >    <span class="missing-if-branch" title="if path not taken" >I</span>if (!srcset) {</span>
<span class="cstat-no" title="statement not covered" >      return</span>
    }
    const resolvedSrcset = <span class="cstat-no" title="statement not covered" >srcset</span>
...
<span class="cstat-no" title="statement not covered" >    <span class="missing-if-branch" title="if path not taken" >I</span>if (img.src.length &gt; MAX_STR_LEN) {</span>
<span class="cstat-no" title="statement not covered" >      sendPlaceholder(id, img)</span>
    }
<span class="cstat-no" title="statement not covered" >    app.send(SetNodeAttributeURLBased(id, 'src', img.src, app.getBaseHref()))</span>
  }
&nbsp;
  const sendImgError = <span class="cstat-no" title="statement not covered" >app.safe(<span class="fstat-no" title="function not covered" >function (i</span>mg: HTMLImageElement): void {</span>
    const resolvedSrc = <span class="cstat-no" title="statement not covered" >resolveURL(img.src || '') </span>// Src type is null sometimes. - is it true?
<span class="cstat-no" title="statement not covered" >    <span class="missing-if-branch" title="if path not taken" >I</span>if (isURL(resolvedSrc)) {</span>
<span class="cstat-no" title="statement not covered" >      app.send(ResourceTiming(app.timestamp(), 0, 0, 0, 0, 0, resolvedSrc, 'img', 0, false))</span>
    }
  })
&nbsp;
  const sendImgAttrs = <span class="cstat-no" title="statement not covered" >app.safe(<span class="fstat-no" title="function not covered" >function (i</span>mg: HTMLImageElement): void {</span>
    const id = <span class="cstat-no" title="statement not covered" >app.nodes.getID(img)</span>
...
<span class="cstat-no" title="statement not covered" >    <span class="missing-if-branch" title="if path not taken" >I</span>if (!img.complete) {</span>
<span class="cstat-no" title="statement not covered" >      return</span>
    }
<span class="cstat-no" title="statement not covered" >    if (img.naturalHeight === 0 &amp;&amp; img.naturalWidth === 0 &amp;&amp; !isSVGInFireFox(img.src)) {</span>
<span class="cstat-no" title="statement not covered" >      sendImgError(img)</span>
    } else <span class="cstat-no" title="statement not covered" >if (app.sanitizer.isHidden(id) || app.sanitizer.isObscured(id)) {</span>
<span class="cstat-no" title="statement not covered" >      sendPlaceholder(id, img)</span>
    } else {
<span class="cstat-no" title="statement not covered" >      sendSrc(id, img)</span>
...
<span class="cstat-no" title="statement not covered" >    <span class="missing-if-branch" title="if path not taken" >I</span>if (!hasTag(node, 'img')) {</span>
<span class="cstat-no" title="statement not covered" >      return</span>
    }
<span class="cstat-no" title="statement not covered" >    app.nodes.attachNodeListener(node, 'error', <span class="fstat-no" title="function not covered" >() =</span>&gt; <span class="cstat-no" title="statement not covered" >sendImgError(node))</span></span>
<span class="cstat-no" title="statement not covered" >    app.nodes.attachNodeListener(node, 'load', <span class="fstat-no" title="function not covered" >() =</span>&gt; <span class="cstat-no" title="statement not covered" >sendImgAttrs(node))</span></span>
<span class="cstat-no" title="statement not covered" >    sendImgAttrs(node)</span>
<span class="cstat-no" title="statement not covered" >    observer.observe(node, { attributes: true, attributeFilter: ['src', 'srcset'] })</span>
  })
}
&nbsp;</pre></td></tr></table></pre>
